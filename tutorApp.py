...
es_ES = {"select_lms": "Selecciona Plataforma: ",
         ...
         "files": "Archivos",
         "about": "Sobre la App",
         "about_content": {"tabs_names": {1: "Instrucciones",
                                          2: "Tipos de Archivos"},
...
en_US = {"select_lms": "Select LMS: ",
         "select_script": "Select Script:",
         "run": "RUN",
         "user": "Credentials",
         "files": "Files",
         "about": "About",
         "about_content": {"tabs_names": {1: "Instructions",
                                          2: "File Types"},
...
class UNABScripts(QMainWindow):
    def __init__(self):
        ...
        self.TabsWidget = TabsWidget(self, data)
...
class TabsWidget(QWidget):
    def __init__(self, parent, savefile=None):
        ...
        self.cboxScriptLabel.setFixedWidth(len(self.lang_dict["select_script"]) * 6)
        ...
        self.user_bb.layout.addWidget(self.bb_pass_btn, alignment=Qt.AlignRight)
        ...
        self.about_tab1_body = QLabel(self.lang_dict["about_content"]["tab1"], self.about_tab1)
        ...
        self.about_tab2_body = QLabel(self.lang_dict["about_content"]["tab2"], self.about_tab2)
        ...
        self.about_tab1.layout.addWidget(self.about_tab1_body, alignment=Qt.AlignTop)
        self.about_tab1.setLayout(self.about_tab1.layout)
        self.about_tab2.layout.addWidget(self.about_tab2_body, alignment=Qt.AlignTop)
        self.about_tab2.setLayout(self.about_tab2.layout)

        self.about_tabs.addTab(self.about_tab1, self.lang_dict["about_content"]["tabs_names"][1])
        self.about_tabs.addTab(self.about_tab2, self.lang_dict["about_content"]["tabs_names"][2])
    ...
    def csv_path_get(self):
        self.csv_path.setText(str(QFileDialog.getOpenFileName(self.csv_path, self.lang_dict["select_file"],
    ...
    def exec_script(self):
        ...
        if self.cboxScript.currentText() == self.lang_dict["script_options"][1]:
            worker = Worker(BbScripts, username, password, path=self.csv_path.text(),
                            func=1, logger=logging, output=self.send_output)
        elif self.cboxScript.currentText() == self.lang_dict["script_options"][2]:
            worker = Worker(BbScripts, username, password, path=self.csv_path.text(),
                            func=2, logger=logging, output=self.send_output)
        elif self.cboxScript.currentText() == self.lang_dict["script_options"][3]:
            worker = Worker(BbScripts, username, password, path=self.csv_path.text(),
                            func=3, logger=logging, output=self.send_output)
        elif self.cboxScript.currentText() == self.lang_dict["script_options"][4]:
            worker = Worker(BbScripts, username, password, path=self.csv_path.text(),
                            func=4, logger=logging, output=self.send_output)
        elif self.cboxScript.currentText() == self.lang_dict["script_options"][5]:
            worker = Worker(BbScripts, username, password, path=self.csv_path.text(),
                            func=5, logger=logging, output=self.send_output)
        self.threadpool.start(worker)

    def save_data(self):
        return {"user": self.bb_username.text(),
